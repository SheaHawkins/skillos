...
SkillOS is a framework for self-evolving agents, based on [Google's SkillOS paper](https://arxiv.org/abs/2605.06614). Agents accumulate experience as structured **skills** stored in a repository, and a **curator** analyses execution traces to keep that repository up to date.
...
| [`Curator`](api/skillos-core.md#curator) | Abstract base class. Receives a `Trace` and returns a `Changelog` of mutations. |
| [`Trace`](api/skillos-core.md#trace) | Thin wrapper around an OpenTelemetry trace (trace ID + spans). |
