...
def tools(repo: SkillRepo):
    return {t.name: t.func for t in create_skill_tools(repo)}
...
def test_insert_invalid_name_raises(tools) -> None:
    with pytest.raises(ValueError):
        tools["insert_skill"](name="BAD NAME", description="desc", body="body\n")


def test_read_skill(tools) -> None:
    tools["insert_skill"](name="hello", description="A greeting.", body="# Hello\n")
...
def test_update_body(tools, repo: SkillRepo) -> None:
    tools["insert_skill"](name="hello", description="desc", body="v1 body\n")
...
def test_update_leaves_unspecified_fields(tools, repo: SkillRepo) -> None:
    ...
    tools["update_skill"](name="hello", description="updated")
    skill = repo.read("hello")
...
def test_update_missing_raises(tools) -> None:
    with pytest.raises(FileNotFoundError):
        tools["update_skill"](name="nope", description="x")
...
def test_tools_have_descriptions(repo: SkillRepo) -> None:
    for t in create_skill_tools(repo):
        assert t.description, f"{t.name} missing description"


def test_full_lifecycle(tools, repo: SkillRepo) -> None:
    tools["insert_skill"](name="alpha", description="first", body="body a\n")
    tools["insert_skill"](name="beta", description="second", body="body b\n")
    assert tools["list_skills"]() == ["alpha", "beta"]

    tools["update_skill"](name="alpha", description="updated first")
    assert repo.read("alpha").description == "updated first"

    tools["delete_skill"](name="beta")
    assert tools["list_skills"]() == ["alpha"]

    result = tools["read_skill"](name="alpha")
